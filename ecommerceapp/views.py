...
def profile(request):
    ...
    for item in items:
        order_status = OrderUpdate.objects.filter(order_id=item.order_id)
        status.append(order_status)

    # Pagination
    paginator = Paginator(items, 5)  # Adjust the number of items per page as needed
    page_number = request.GET.get('page')
    page_obj = paginator.get_page(page_number)

    context = {"items": page_obj, "status": status}  # Pass page_obj instead of items
